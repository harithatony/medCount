...
def main_page():
    ...
    st.session_state.after_list = []
    st.session_state.before_list = []
...
def count_page():
    ...
    with col1:
        ...
        uploaded_files_col1 = [st.file_uploader(f"Before Surgery {i}", type=["jpg", "jpeg", "png"]) for i in range(1, 6)]
        before = "Before Image"
        before_list.append(display_uploaded_images(uploaded_files_col1, col1,before))

    # Upload images for the second column
    with col2:
        st.header("After Surgery")
        uploaded_files_col2 = [st.file_uploader(f"After Surgery {i}", type=["jpg", "jpeg", "png"]) for i in range(1, 6)]
        after = "After Image"
        after_list.append(display_uploaded_images(uploaded_files_col2, col2,after))
